const PM_SITES_KEY = "pm_sites_v3";
const LEGACY_SITE_KEYS = ["pm_sites_v2","pm_sites_v1"];
...
function getApprovedSites(){
  try{
    let sites = JSON.parse(localStorage.getItem(PM_SITES_KEY) || "null");
    if (!Array.isArray(sites)) {
      for (const key of LEGACY_SITE_KEYS) {
        const legacy = JSON.parse(localStorage.getItem(key) || "null");
        if (Array.isArray(legacy)) { sites = legacy; break; }
      }
    }
    if (!Array.isArray(sites)) return [];
    return sites.filter(s => s && String(s.status).toLowerCase() === "approved");
  }catch(e){ return []; }
}
